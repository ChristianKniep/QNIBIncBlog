...
I added `cpu:broadwell` above as a hardware feature, but that is not recognise by runtimes unless you patch it.
...
*Just take a breath*: Depending on how the machines login the runtime will be presented with the right choice. :)
...
Downloading and running the image will provide us with a (fake) image that is 'optimized' for Broadwell CPUs.

...
## MELIG #2
In the course of the second metope I introduced docker compose, tipped my toe into Consul, showed of how elasticsearch differs from other data-bases and gave an extensive introduction of a logstash base log analytics stack. Towards the end we talk about how to combine metrics and logs to get the best of it.

## Youtube [german]

Since we got now english speaker in the crowd, we went for german. 
The quality is not the best to begin with, I am working on a capturing device to get a better quality screen sharing. Unfortunately most of the consumer devices are meant to work with Windows - I have to find out what that 'Windows' refers to. :)
...
The WebUI (`<dockerhost>:8500`) shows two nodes, one got one server (the consul service that is).
...
By hitting the loadbalancer at port `8000` a cat is presented. The load-balancer logs to syslog and thus to logstash.

Through Kibana (port `80`) one discovers the nginx log.
...
By checking out the tag and removing the stub-file that triggers a logstash reload, the change is applied.
...
There are still a lot of packages which are empty messages, furthermore the logging facility `supervisord` is misleading, since it's just the init system and the real program is `nginx`. The next tag addresses that.
...
After the demo I driften a bit into how to bundle a metrics and a log backend. I keep this piece in the bucket as a different blog entry (/ MeetUp). :
